...
## Get emailed when a new order comes in

By default you'd have to check `data/orders.json` manually. To get an email
instead, this uses [Resend](https://resend.com) — a simple transactional
email API with a free tier.

1. Create a free account at [resend.com](https://resend.com).
2. Go to **API Keys** and create one. Copy it (starts with `re_...`).
3. Add two environment variables — locally in `.env`, and on Render under
   **Environment**:
   ```
   RESEND_API_KEY=re_your_key_here
   NOTIFY_EMAIL=you@example.com
   ```
   `NOTIFY_EMAIL` is where the alert gets sent — use your own inbox.
4. That's it — no domain setup needed to start. Resend lets you send from
   `onboarding@resend.dev` immediately. (Once you have your own domain,
   you can verify it in Resend and set `NOTIFY_FROM_EMAIL` to send from
   your own address instead.)

If you skip this section entirely, the store still works fine — orders
just won't trigger an email, and you'd check `data/orders.json` yourself.
